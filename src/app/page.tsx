...
export default function Page() {
  ...
  const checkAuthAndExecute = (action: () => void) => {
    if (!isLogged) { setShowAuthModal(true); return; }
    action();
  };

  const handleMenuAction = (type: string) => {
    setActiveMenu(type === "imagem" ? "upload" : "none");
  };
      ...
      <main className="relative flex flex-col items-center justify-center min-h-[100dvh] max-w-[584px] mx-auto px-4 pt-20 pb-28 sm:pt-[140px] sm:pb-24 z-10" >
        ...
        <div className="relative w-full">
          <div className="hidden sm:block">
            ...
            {activeMenu === "upload" && (
              <div className="absolute bottom-full left-0 mb-4 z-[999]">
                <FileUpload onFileSelect={handleFileUpload} onClose={() => setActiveMenu("none")} />
              </div>
            )}
            {activeMenu === "preview" && previewUrl && (
              <div className="absolute bottom-full left-0 mb-4 z-[999]">
                <ImagePreview image={previewUrl} onCancel={cancelPreview} onConfirm={confirmUpload} />
              </div>
            )}
          </div>
        ...
        <div className="flex sm:flex-wrap overflow-x-auto sm:overflow-visible no-scrollbar snap-x snap-mandatory justify-start gap-3 mt-6 w-full px-2 py-1 -mx-2">
          {HOME_SUGGESTIONS.map((item) => (
            <div key={item.label} className="snap-start shrink-0">
              <SuggestionChip label={item.label} active={item.active} onClick={() => handleSuggestionClick(item.label)} />
            </div>
          ))}
        </div>
      </main>
