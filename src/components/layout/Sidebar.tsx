...
export const Sidebar = ({ isOpen = false, onClose, onUploadClick, onManageFilesClick }: SidebarProps) => {
  ...
  useEffect(() => {
    setIsMounted(true);
    loadHistory();
    if (user?.role === "professor") loadRecentFiles();
  ...
  const sidebarContent = (
    <>
      <div className={`flex items-center mb-6 w-full ${isExpanded ? "justify-between" : "justify-center"}`}>
        {isExpanded && (
          <span className="text-neutras-50 font-bold whitespace-nowrap">Menu Principal</span>
        )}
        <Button
          variant="ghost"
          size="icon"
          onClick={isMobile ? onClose : () => setIsExpanded(!isExpanded)}
          className="text-neutras-50 hover:bg-neutras-800 shrink-0 min-w-[44px] min-h-[44px]"
        >
          {isMobile ? <X size={24} weight="bold" /> : <List size={24} weight="bold" />}
        </Button>
      </div>
                ...
                recentFiles.map((file) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      {isExpanded && <span className="text-neutras-50 text-sm font-medium truncate">{file.name}</span>}
                    </div>
                    {isExpanded && (
                      <span className="text-neutras-500 text-[10px] ml-6 mt-1">{formatShortDate(file.uploadDate)}</span>
                    )}
                  </div>
                ))
              )}
                ...
                history.map((item) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      {isExpanded && <span className="text-neutras-50 text-sm font-medium truncate">{item.topic}</span>}
                    </div>
                    {isExpanded && (
                      <span className="text-neutras-500 text-[10px] mt-1">{formatShortDate(item.date)}</span>
                    )}
                  </div>
                ))
              )}
      ...
      <div className={`w-full mt-6 pt-6 border-t border-neutras-800 flex ${isExpanded ? "items-center justify-between" : "flex-col items-center gap-4"}`}>
        ...
        {isMounted && user && (
          <Button
            ...
            title="Sair da conta"
            className="text-neutras-400 hover:text-neutras-50 min-w-[44px] min-h-[44px]"
          >
    ...
    <>
      {isMobile && (
        <>
          {isOpen && (
            <div
              className="fixed inset-0 z-30 bg-black/50 backdrop-blur-[2px] md:hidden animate-in fade-in duration-200"
              onClick={onClose}
            />
          )}
          <aside
            className={`
              fixed top-0 left-0 h-full z-40 w-[300px]
              bg-neutras-900 border-r border-neutras-800
              flex flex-col p-6
      ...
      {!isMobile && (
        <aside
          className={`
            h-screen bg-neutras-900 border-r border-neutras-800
            flex-col p-6 z-20 shrink-0
            transition-all duration-300 ease-in-out
            hidden md:flex
            ${isExpanded ? "w-[325px]" : "w-[88px] items-center"}
          `}
        >
          {sidebarContent}
        </aside>
